...
function draw() {
  ...
  d3.json("/getData", function (error, data) {
    ...
    countOfBars = Math.max(
      data.length / 60,
      Object.keys(queryResults[1]).length - 2
    );

    bandwidthFlag = 1;
    if (countOfBars === 1) bandwidthFlag = 0;
    ...
    var yAxisText =
      "Indicators" + '"' + Object.keys(queryResults[1])[maxColumn] + '"';
    ...
    g.selectAll(".bar.country1")
      ...
      .attr(
        "x",
        (d) => x0(d.year) + bandwidthFlag * (x0.bandwidth() / countOfBars)
      )
    ...
    if (data.length > 120) {
      g.selectAll(".bar.country3")
        ...
        .attr(
          "x",
          (d) =>
            x0(d.year) + (bandwidthFlag + 1) * (x0.bandwidth() / countOfBars)
        )
        .attr("y", (d) => y(d[Object.keys(queryResults[1])[2]]))
        .attr("width", x0.bandwidth() / countOfBars)
        .attr("fill", "red")
    ...
    if (Object.keys(queryResults[1]).length > 3) {
      g.selectAll(".bar.indicator2")
        ...
        .attr("x", (d) => x0(d.year))
        .attr("y", (d) => y(d[Object.keys(queryResults[1])[3]]))
        .attr("width", x0.bandwidth() / countOfBars)
        .attr("fill", "steelblue")
    ...
    if (Object.keys(queryResults[1]).length > 4) {
      g.selectAll(".bar.indicator3")
        ...
        .attr("y", (d) => y(d[Object.keys(queryResults[1])[4]]))
        .attr("width", x0.bandwidth() / countOfBars)
        .attr("fill", "black")
